 ...
 * targets to served PNGs — already-raster figures copied through, vector
 * PDF/EPS rasterized via the injected {@link LatexRasterPort} (the mupdf /
 * Ghostscript calls are the only non-filesystem side effects; they live in
 * infra). Output lands in the doc's `assets/` dir and is served by
 * `/images/<doc_id>/<file>` like the other pipelines.
 *
...
import { isFile, rglobFiles, withinTree } from "./fs-util.js";
import type { LatexRasterPort } from "./ports.js";
...
export interface AssetResolverOptions {
  ...
  enabled?: boolean;
  log?: (msg: string) => void;
  /** Vector rasterizers; without one, vector figures resolve to undefined. */
...
export class AssetResolver {
  ...
  private readonly raster?: LatexRasterPort;
  ...
  private resolve(arg: string): string | undefined {
    ...
    try {
      ...
      if (
        VECTOR_PDF.has(ext) &&
        this.raster?.pdfToPng(src, dest, { dpi: this.dpi, maxPx: this.maxPx })
      ) {
        return basename(dest);
      }
      if (
        VECTOR_EPS.has(ext) &&
        this.raster?.epsToPng(src, dest, { dpi: this.dpi, log: this.log })
      ) {
        return basename(dest);
      }
