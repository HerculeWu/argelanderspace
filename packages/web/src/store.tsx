...
// re-measure once the scroll position is stable and correct. Instant
// corrections fired mid-animation get swallowed by the still-running smooth
// scroll (real-Chrome evidence), and `scrollend`/fixed timers both lose to
// that — so the trigger is settle detection: sample scrollTop, re-arm while
// it keeps changing, only correct once it holds still. Bounded: ≤2
// corrections, ≤8 re-arms; cancelled by user scroll input / undo / a new jump.
const JUMP_CORRECT_MS = 900;
const JUMP_RECHECK_MS = 400;
const JUMP_REARM_MS = 250;
const JUMP_CORRECT_MAX = 2;
const JUMP_REARM_MAX = 8;
...
export function StoreProvider({ ir, children }: { ir: DocIr; children: React.ReactNode }) {
  ...
  const store = useMemo<StoreValue>(() => {
    ...
    // Landing correction: re-measure the target once the scroll position is
    // STABLE — instant corrections fired while the smooth scroll is still
    // running get swallowed by it (real-Chrome evidence), and scrollend/
    // fixed timers both lose to that, so the trigger is settle detection:
    // sample scrollTop, re-arm while it keeps changing (without burning a
    // correction attempt), correct once it holds still.
    const scheduleJumpCorrect = (el: HTMLElement, attempt: number, rearm = 0) => {
      cancelJumpCorrect();
      const root = readerEl.current;
      if (root === null) return;
      const sample = root.scrollTop;
      const delay =
        attempt === 1 && rearm === 0
          ? JUMP_CORRECT_MS
          : rearm > 0
