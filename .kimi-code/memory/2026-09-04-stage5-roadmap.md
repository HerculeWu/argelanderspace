...
**2026-09-09：Stage 5 关闭——smoke 全过、已 push。** smoke R1 修复 `6bb4576`（图转换器 dvisvgm→pdftocairo+gs，Q4/Q9 修订）后用户复验通过；迁移备份 tarball 已删。遗留 4 项进 `2026-09-01-known-issues.md`「下一阶段重点」（宽度自适应/多引用折行/右栏定位/参考文献元数据）。commit 序列（main）：定稿 memory `edd8fb6` → MS1 `d91471d` → MS1 复核 `e4548b5` → MS2 memory `26e0632` → MS2 `ec8ffb7` → MS3a memory `0c20803` → MS3a `8e31a04` → MS3b memory `74b34b1` → MS3b `0db29c7` → MS4a `a8179d3` → MS4b `3a101e9` → smoke R1 `6bb4576` → 收尾 memory（本次）。
