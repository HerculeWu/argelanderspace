...
describe("RefDetail document provenance and actions", () => {
  it("shows each Doc's trusted source, identifier, and independently selected main position", async () => {
    ...
    const openDoc = screen.getAllByRole("button", { name: "打开此文档" })[1]!;
    expect(openDoc.getAttribute("data-ui")).toBe("open-doc");
    expect(openDoc.textContent).toBe("");
    const tooltip = document.getElementById(openDoc.getAttribute("aria-describedby") ?? "");
    expect(tooltip?.textContent).toBe("打开此文档");
  ...
  it("guards same-tick PDF submissions and leaves the large-file warning cancellable", async () => {
    ...
    const entry = document.querySelector<HTMLButtonElement>('[data-ui="gether-doc"]')!;
    expect(entry.closest('.ref-files-head')?.querySelector('h2')).toBeTruthy();
    expect(document.querySelector('[data-ui="open-fulltext-acquisition"]')).toBeNull();
    fireEvent.click(entry);
    expect(screen.getByRole("dialog", { name: "获取全文" })).toBeTruthy();
    fireEvent.click(screen.getByRole("radio", { name: /上传本地 PDF/ }));
    let input = document.querySelector('[data-ui="pdf-upload-file"]') as HTMLInputElement;
    ...
    expect((screen.getByRole("button", { name: "上传 PDF 并添加正文" }) as HTMLButtonElement).disabled).toBe(true);
    expect(fetchMock.mock.calls.some(([url]) => String(url).includes("upload-pdf"))).toBe(false);
    fireEvent.click(document.querySelector('[data-ui="confirm-pdf-resource-warning"]')!);
    expect((screen.getByRole("button", { name: "上传 PDF 并添加正文" }) as HTMLButtonElement).disabled).toBe(false);
    fireEvent.click(screen.getByRole("button", { name: "取消" }));
    expect(screen.queryByText(/超过 25 MiB 提示阈值/)).toBeNull();

    fireEvent.click(entry);
    fireEvent.click(screen.getByRole("radio", { name: /上传本地 PDF/ }));
    input = document.querySelector('[data-ui="pdf-upload-file"]') as HTMLInputElement;
    ...
    const submit = screen.getByRole("button", { name: "上传 PDF 并添加正文" });
