...
| Stage 14 / 2026-09-17 | ADS 文献发现（Discovery）替代旧全局推荐：`GET /api/library/discovery`（ADS similar 18 + useful 6 基于实际 related 集，真实引用边，只读无锁无 WS，ads-discovery 24h TTL 缓存）；文献页内 Explore 模式 + `#explore/<bibcode>` hash（仅成功 seed 跳转进历史、失败保留旧图、真取消）；共享图 renderer + Library/Discovery 双 adapter（箭头/标签避让/tooltip/菱形 Useful）；入库只走 works bibcode 路径；候选无 BibTeX tab；库图 saved-only + graph.json v2 锁内惰性自愈（失败显式 500）；退役 POST /api/library/refs、addNodeToLibrary、GraphNodeDetail、OpenAlex fetchMany、推荐开关/滑块、exportBibtex 假占位（Stage 13 遗留 Q1 了结）；CLI 冻结面不动。smoke 顺手修摘要渲染（provider HTML 白名单 + 文本节点 KaTeX，abstractHtml）。四门 1173+1、真实 Chrome 43/43、真实 ADS smoke 17/17（18 related/6 useful/129 边）；**用户 smoke 通过** | feat 提交见本提交；阶段方案 memory-reference/stage14-discovery-plan.md（已关闭） |
| Stage 15 / 2026-09-18 | 添加文献时自动导入 arXiv 正文：works 路由创建/exists 后按场景表自动排队 `"ingest"` job（预留枚举启用；A 新挂主 doc / B 同 id 恒新鲜下载原位刷新 / C 只有用户上传则不碰；identifier+bibcode 模式含 Discovery 继承，批量 bib 永不）；新端点 `POST /api/library/attach-arxiv`（手动与自动同一实现）；挂载照 upload 链路（`acquired_via="arxiv_eprint"` 焊死+锁内 rebuild+reassert+终检）；`ArxivPdfOnlyError` 类型化（文案不变，CLI 零变化）+ `JobSchema.errorCode`；`logs/arxiv-fetch.jsonl` 报错日志（只记 arXiv 来源失败/中断，纯文本直读，用户定为迭代重要参考）；附件 tab 可点击行（原样式）+进度/重试、arXiv doc 行「重新获取」、获取类广告标签清理（留来源/待上传）；隐藏开关 `auto_ingest_arxiv` 默认开；CLI 不动（I035 推后）。四门 1216+1、真实 Chrome+真实网络探针 26/26（真 ADS/真 arXiv/失败重试日志）；**用户 smoke 通过** | feat 提交见本提交；阶段方案 memory-reference/stage15-arxiv-auto-ingest.md（已关闭）；grilling 原文 .pi/inbox/2026-09-17-stage15-arxiv-auto-ingest.md（随 docs 批入 Git） |

Stage 11 的编号由 2026-09-16 用户 Q1 明确统一（原 session D1，持久定位见提交 `5569f98` 的 `.pi/inbox/2026-09-16-mem-merge-stage11.md`）；原会话 D17 已明确阶段关闭但未写正式编号。旧 Stage 11 scope 是草案输入，不是下一轮仍需自动实施的计划。Stage 4.1 仍推后。Stage 13（webui 手动建条目）已关闭，决策记录见同批 docs 提交的 `.pi/inbox/2026-09-16-01a0abdd-stage13-kickoff.md`。Stage 14（ADS 文献发现）已关闭，grilling 决策与验收证据原文见本批提交的 `.pi/inbox/2026-09-17-01a0aefe-stage14-grilling.md`。Stage 15（添加文献时自动导入 arXiv 正文）已关闭，决策与验收见 `.pi/inbox/2026-09-17-stage15-arxiv-auto-ingest.md`（随关闭 docs 批入 Git）。**下一阶段未指定**（等用户拍板）。
