...
当次用户授权实施一组票据时，按批准的依赖顺序逐票执行：**每张票据启动一个独立 fresh-context 子代理**，只向它交付该票范围、完整规格与前置验收证据。父代理逐票核对结果与验收门；前票未完成、测试未通过或需要用户决策时，记录阻塞、停止后续票据并询问用户，不借多票授权跳过人工停止点。子代理负责执行，不代替父代理决定范围、接受证据或提交权限。单次运行限时及到期由主代理介入的正式规范见[工程指南](../engineering.md#子代理实施限时与主代理介入)。调度方式遵守当前子代理工具协议；本约定不授予额外的数据迁移、提交或发布权限。
