...
**2026-09-10：MS4（CLI 未识别源建条目 B）landed。** commit `0d7c8c3`：detectSource 判别联合（DOI/URL→doi-stub；URL 仅路径字面携带 DOI 才建——OUP/A&A 无 DOI 路径维持报错；裸字符串维持报错）；core `addDoiWork`（origin `"manual"`、Crossref 即时富化 + 失败裸条目降级 exit 0、DOI 去重零写盘、title bridging 归并、**不触发 rebuild**——就地 planFor + cite_key 只增分配，同 addNodeToLibrary 先例）；`10.48550/arXiv.*` 路由回 latex 管线（不建垃圾 stub）；`doi:` 前缀输入。对抗审查 APPROVE（0 阻断，8 非阻断全处置：尾斜杠/句点/括号粘贴剥离、URL 来源无 Crossref 记录明示、flag 静默忽略改 stderr note、arxivFromDoi 复用 core、SICI 截断记录）。webui attach 对 `doi:<doi>` id 实测命中（findWork workId/doi 两路）。测试 core 202→209、cli 33→40，四门全绿。

**2026-09-10：MS3（re-upload 推广 A1）landed。** commit `facf09d`：上传按钮全量开放（三态文案：无 doc"上传正文"/有 upload doc"重新上传覆盖同一文档"/有非 upload doc"上传新版本…旧版本保留可回看"）；attach 从 append 改 **prepend 设主**（主 doc = `doc_ids[0]`，全链路现状沿用；seed 归并 dst-first 保序 rebuild 不丢主位）；LibraryRef payload 补 optional `doc_ids`（**核实：CLI agent.ts 直读磁盘 LibraryStore、无 HTTP 消费 LibraryPayload，agent 字节冻结守住**）；主 doc 切换 = PATCH 透传面加 `doc_id`（move-to-front，server 零源码改动）；web RefDetail 版本列表 + "设为主"（失败显错不重载）。对抗审查 APPROVE（0 阻断）：N1 weld 竞态（prepend 可被并发 PATCH 覆盖 → lockedRebuild 后锁内 re-assert 主位，server 注入 libraryLock 版）与 N5（no-op patch 不 save 返回 false）已修；**N2 从零 rebuild 丢主位**（library.json 丢失时字母序播种，"upload 是主"无持久标记）/**N3 stale doc 在下次 rebuild 前全量曝光**（版本列表 N 个 404 条目）/**N4 bridging 塌缩方向决定主位存亡** → 挂 known-issue。测试 core 197→202、server 91→94、web 142→144，四门全绿。
