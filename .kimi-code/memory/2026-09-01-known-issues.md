...
- **subequations 被 pandoc 合并成一条 DisplayMath**（3.1.3/3.9 同）：只出 1 个 equation block、只编 1 号，`\label` 前缀在 `ENV_RE` 贪婪匹配外静默丢弃。拆开需理解合并结构，发生率低——2026-09-01 grilling Q18 拍板不修，立此存照。**（pandoc 特有；Stage 5 管线替换后此条目随 pandoc 退役，届时按新管线行为重评估）**
