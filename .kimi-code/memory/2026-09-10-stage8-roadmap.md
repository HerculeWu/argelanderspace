...
**2026-09-12：Stage 8 关闭——MS1–MS6 全部 landed、用户 smoke 验收通过、已 push origin main。** commit 序列（main）：定稿 `cc16c4e` → MS1 `a01e84c`（contracts+core 数据层）+ memory `b15926f` → MS2 `b586d61`（annotations REST + 文档删除）+ memory `62a367d` → MS3 `45bbcd4`（web 标注 CRUD shell）+ memory `4d63386` → MS4 `9546fc0`（文本选区 + 重叠高亮引擎）+ memory `126c30a` → MS5 `d34d4ac`（CLI annot + skills）+ memory `a5f9b48` → MS6 `13618e7`（smoke 手册 + memory 收尾）→ 收尾 memory（本次）。**硬约束守住**：agent 侧既有输出逐字节零改动（golden 零重冻、`agent.ts` 纯增量）；`annot` 输出自诞生冻结。遗留/存照进 `2026-09-01-known-issues.md` Stage 8 节。**下一阶段由用户拍板**（Stage 4.1 仍推后；候选见 known-issues 各节与各 roadmap 推后事项）。

**2026-09-10：MS6（smoke 手册 + memory 收尾）landed `13618e7`。** `docs/manual-test-stage8.md`（§0–11：环境/文本标注/结构标注/文档级/右栏 tabs/深链接/失效归档/CLI annot/文档删除/外部编辑/已知边界/回归面），关键预期全部真浏览器预实测——副本 `/tmp/stage8-smoke`（754MB `cp -a`，真实库 mtime 校验零改动）+ 真实 Chrome raw-CDP 探针 9 个（throwaway，用后已删未入库，设计如此；截图 25 张留档 `/tmp/stage8-smoke/shots/`）。实测新确认：列表条目点击 = jump+flash+激活**不开 popover**（定稿 §7 原文如此，手册按实写）；upload docId = `upload-<slug44>-<workId sha1 6>` 只随 work 变（重传改内容即原位覆盖 → 失效测试通道）；arxiv 同字节重摄入（`.latexcache` 缓存零网络）标注存活；14MB 真实 zip 制造长 busy 窗口验 409。与定稿字面的出入全部按实入手册（无产品 bug）。四门全绿（51/266/60/248/135/50）。

**2026-09-10：MS5（CLI `annot` + skills）landed `d34d4ac`。** `annot <doc_id>` 真只读（指纹自算：match→JSONL / mismatch→空 stdout+固定 stderr note `note: document content changed since its annotations were written; they are archived and not shown`，全程零写盘零 archive——归档权威仅在 server 访问路径）；冻结行契约 = 键序 `id,doc_id,target,context,body,created_at,updated_at,link` + 文档阅读序（与 web `sortAnnotations` 500 轮 fuzz 对拍零分歧）+ `#ann-<id>` link；context 分级（document→title / section·float→section_id+section_path / text·paragraph·list→+完整 canonical container_text）。skills/read-paper 命令面加行 + skills/README + 根 README 登记。对抗审查 APPROVE / 2 非阻断已修（错误路径先物化全行再打印——悬空 target 不再吐部分行；SKILL.md 字段集补齐 8 个）。存照：corrupt IR 报错无文件路径（既有 helper、冻结约束）；CLI 无 docId 穿越防护（与 read/show/ref 同信任模型）。测试 cli 40→50，四门全绿。**agent 侧既有输出零改动（冻结面守住）。**
