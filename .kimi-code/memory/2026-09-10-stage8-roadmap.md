...
**2026-09-10：MS6（smoke 手册 + memory 收尾）landed（本 entry 随手册先行 commit，待 smoke）。** `docs/manual-test-stage8.md`（§0–11：环境/文本标注/结构标注/文档级/右栏 tabs/深链接/失效归档/CLI annot/文档删除/外部编辑/已知边界/回归面），关键预期全部真浏览器预实测——副本 `/tmp/stage8-smoke`（754MB `cp -a`，真实库 mtime 校验零改动）+ 真实 Chrome raw-CDP 探针 9 个（throwaway，用后已删未入库，设计如此；截图 25 张留档 `/tmp/stage8-smoke/shots/`）。实测新确认：列表条目点击 = jump+flash+激活**不开 popover**（定稿 §7 原文如此，手册按实写）；upload docId = `upload-<slug44>-<workId sha1 6>` 只随 work 变（重传改内容即原位覆盖 → 失效测试通道）；arxiv 同字节重摄入（`.latexcache` 缓存零网络）标注存活；14MB 真实 zip 制造长 busy 窗口验 409。与定稿字面的出入全部按实入手册（无产品 bug）。四门全绿（51/266/60/248/135/50）。**待用户 smoke**——stage 关闭 entry（含全 commit 清单）待 smoke 通过后补：

- （占位，待 smoke）Stage 8 关闭：用户验收 + push + 全 commit 清单定稿。
