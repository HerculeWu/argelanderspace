...
- `ActionButton` is the public shared action interface when code chooses between `mode="text"` and `mode="icon"`. Text mode renders only its supplied text; icon mode renders only an icon, with a text accessible name and shared custom tooltip. Supply localized `label`/`tooltip` strings from the existing locale JSON; a missing or rejected configured icon becomes a visible text button rather than an empty or misleading control. For an existing control with its own established appearance, pass `unstyled` to preserve its local height, borders, background and typography instead of layering `.btn.ui-button`; the shared accessible name, tooltip, disabled and busy behavior still apply. A tooltip wrapper around a flex item or absolutely positioned action must preserve the original layout and positioning. The PDF annotation destination is a deliberate mixed icon + page-number control (not an `ActionButton` mode); its kind icon comes from the same restricted resource parser.
- Icon-mode resources come only from `src/ui/icons.json`, a single language-neutral map **keyed by each control's `data-ui`** so a user can find the exact SVG. If one identifier represents state-dependent icons, its value is a map from the code's `iconName` variant to SVG strings; a single-icon action's value is an SVG string. This deliberate duplication makes each action directly editable. Resource data contains no action or presentation mode and is rendered only after `icon-resource.tsx` parses the restricted inline SVG subset (fixed 24×24 viewBox; basic path/shape elements and allowlisted presentation/geometry attributes). Scripts, event attributes, styles, external references, XML declarations/entities, and unsupported SVG content are rejected. Do not put arbitrary HTML/SVG into the DOM or bypass this parser.
- `Tooltip` shows on pointer entry or keyboard focus, and dismisses on activation, pointer exit, blur or Escape. Only one tooltip is visible at a time, including when the pointer hovers one button while keyboard focus moves to another. A pointer click must not leave its focused button's tooltip showing beside the next hovered control; re-entry or a fresh keyboard focus may show it again.
