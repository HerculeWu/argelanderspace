...
- **task.due 可晚于 plan.due**（2026-09-02 smoke 发现，用户明确本阶段不修、记为未解决问题）：TaskModal/schema 均不校验 task.due ≤ plan.due；若要约束需先拍板语义（计划延期正当场景 vs 数据错误），详见 known-issues。
...
- **smoke 第 2 轮修复（2026-09-02，commit `24c77ac`）**：用户报"跨组拖拽还是没有被拒绝"。主 agent 真实 Chrome（playwright-core 驱动系统 Chrome）复现结论：**机制拒绝其实已生效**（0 PUT、顺序不变），用户判"没拒绝"的依据是拖拽过程的视觉受理感（行跟手进别组、源组让位）——真缺口 = **拒绝不可感知**。修复 = UX 三件套：拖动时其他组 `plan-no-target` 变暗 + 全域 `cursor: no-drop`、被拒落点闪 nudge"跨组移动请用状态圆钮或抽屉改状态"（2.5s，`.plan-reject-nudge` 错位到 114px 避开 409 flash、带 view-in）；顺手加固静态缓存头（index.html/SPA fallback `no-cache`、hashed assets `immutable`——排除旧 bundle 干扰路径）。真实 Chrome 验证三件套全生效（截图+DOM 证据）；审查 0 阻断。测试 448→449。用户另报"task.due 可晚于 plan.due"——**本阶段不修，已记 known-issues + 推后事项**。
- **smoke 第 3 轮修复（2026-09-02，commit `8f5338d`）**：用户确认列表拦截生效，但**看不到禁落光标**——真 bug：no-drop 打在目标组元素上，拖拽中被拖行恒覆盖指针正下方，浏览器显示最上层元素的光标，组级规则实际不可见。修复：`onDragMove` 跟踪指针离开本组 → root `plan-nodrop-active` → 禁落光标打到**被拖行及其子元素**（`.dragging, .dragging *`——全局 `button{cursor:pointer}` 的声明值需 `*` 覆盖，审查抓的窄残余）。真实 Chrome 验证：本组内 pointer、越界 no-drop、drop 后复位。审查 0 阻断。**教训**：dnd-kit 拖拽中指针下的元素永远是被拖行——hover 反馈（光标/高亮）必须打到被拖行或 overlay，打在落点目标上不可见。用户另确认：看板跨列拖拽不拦截 = 定稿 Q11 故意的不对称设计（列表拖拽只调序、看板拖拽即改状态），手册 §5b/§12-3 已写精确。
