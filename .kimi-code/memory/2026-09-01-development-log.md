...
## Stage 8（2026-09-10 设计/执行，2026-09-12 关闭）：文档标注（annotations）

- 设计 grilling 三轮 Q1–Q20 全锁定 + 用户终稿修正 4 点（归档权威收窄 server 访问路径/CLI 真只读、DELETE 全局物理删除、三态硬规则原样保留、orphan 不写既成事实），定稿 `2026-09-10-stage8-roadmap.md`。定位：标注 = per-doc 独立用户数据层（目标位置 + 内容；文本允许任意重叠；只要求在其所属 doc 当前内容成立，原位替换 → 整批失效归档）；webui CRUD 闭环不依赖 agent，agent 只读。
- 执行：定稿 `cc16c4e` → MS1 contracts+core `a01e84c`+memory `b15926f`（target 三型/canonical text/指纹投影/ensureCurrent 三态幂等归档；审查 2 阻断全修——literatures 非整体 gitignore 补规则、archive 改字节拷贝保外部未知键）→ MS2 server+删除 `b586d61`+`62a367d`（GET/PUT 双校验 409×2、watcher 第三指纹、DELETE 全局摘除、DocMutationRegistry 生命周期互斥；审查 1 阻断已修——pin 永久泄漏）→ MS3 web CRUD shell `45bbcd4`+`4d63386`（右栏 tabs/八类边钮/popover/深链接/删除按钮+三态迁移；0 阻断 8 非阻断全修；deeplink flake 根治=openView 幂等）→ MS4 文本选区+重叠引擎 `9546fc0`+`126c30a`（textmap 原语/CSS Custom Highlight/原子归一/选择器/gutter 计数；真实 Chrome CDP 探针实证；1 阻断已修——409 把文本创建目标静默转结构标注违不可变哲学→关闭+toast；2080 组 offset fuzz 全等）→ MS5 CLI annot+skills `d34d4ac`+`a5f9b48`（真只读自算指纹、冻结行契约、web/CLI 排序 500 轮 fuzz 对拍零分歧）→ MS6 smoke 手册+memory 收尾 `13618e7`（手册 `docs/manual-test-stage8.md` 关键预期全部真实 Chrome 预实测：选区/原子扩展/重叠/选择器/gutter/拒绝 toast、7 类结构边钮（现库无 algorithm 块）、tabs/排序/编辑/updated_at/深链接/unknown 静默、失效归档全链路（upload+arxiv 双通道存活、CLI mismatch 不归档、archive 字节保全、开编辑器 409 toast+归档 banner）、删除三态+busy 409、外部编辑免 F5）。
- 测试：contracts 51 / core 266 / infra 60 / web 248 / server 135 / cli 50，四门全绿。**硬约束守住：agent 既有输出零改动，annot 自诞生冻结。**
- **Stage 8 关闭（2026-09-12）**：用户 smoke 验收通过，全部 push origin main。遗留/存照进 `2026-09-01-known-issues.md` Stage 8 节（reader 不重取 IR/部分删除 ghost 条目/orphan annotations/跨进程同 doc 并发不支持等 8 项）。

## 下一步

**Stage 6 已关闭（2026-09-09）**：阅读器四项修复（宽度自适应/多引用 per-ref chip/右栏定位停稳校正/当前文献作者块三族抽取）+ 存量离线重摄入迁移 + smoke R1 修复（revtex 粘贴 .bbl 宏汤、\ensuremath KaTeX shim）全部 landed，用户验收通过并 push。定稿/决策/审查记录见 `2026-09-09-stage6-roadmap.md`。

**Stage 7 已关闭（2026-09-10）**：known-issues 清账全部 landed、用户验收通过并 push（定稿/里程碑/审查见 `2026-09-09-stage7-roadmap.md`）。

**Stage 8 已关闭（2026-09-12）**：文档标注全部 landed、用户验收通过并 push（定稿/里程碑/审查见 `2026-09-10-stage8-roadmap.md`）。**下一阶段由用户拍板**。候选：known-issues 各节遗留（5 篇 src-only 缺 arXiv 类文件、脚注 DROP、tikz、作者块残留边界、Stage 8 节 8 项存照）、Stage 4.1（agent 操作计划页面：CLI/skills 读写 `status/plans.json`）**推后**——数据层已预留：稳定 `p_/t_` id、pretty JSON、watcher 覆盖、`plan.changed`、CRUD 纯函数；标注层未来方向（migration skill / 编辑工作流 / 协作 comments 扩展）见 `2026-09-10-stage8-roadmap.md` 产品定位节。
