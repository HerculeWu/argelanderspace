...
**主线是 TypeScript（`ts-migration` 分支，M0–M6）**：pnpm workspace 在 `packages/` 下——`contracts`（zod 契约）/ `core`（纯领域）/ `infra`（mupdf、pandoc、MinerU、ADS/Crossref/OpenAlex、config.toml）/ `server`（Hono + job runner + WS）/ `cli`（commander）/ `web`（React 阅读器）/ `app`（发布用的单包 bundle，`argelanderspace` on npm）。构建/测试/检查一律 `corepack pnpm -r build|test|typecheck|lint`（本机裸 `pnpm` 不在 PATH，必须走 corepack）。

**Python 旧树（`bibgraph/`、`server/`、`tests/run_tests.py`）是 legacy，待删**：保留到用户人工冒烟验收 TS 版之后（git history 仍在）。不要改它，也不要再为它加功能。当前主线的背景与计划见 memory 目录。
...
- Python 解释器 `/home/wwu/miniforge3/envs/astro/bin/python` 仅服务于 legacy 树（删除后此条作废）
