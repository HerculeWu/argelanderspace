...
上传目标必须焊死为用户选择的 Work，而不是重新按标题碰运气归并；主位和 cite key 各自遵守稳定规则。Doc 删除是全局物理生命周期，不是只从一个 Work 解绑。完整行为见 [library](../library.md) 与 [contracts](../contracts.md)，本 ADR 不另写一版数据保护规则。
