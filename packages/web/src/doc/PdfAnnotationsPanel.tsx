...
export function PdfAnnotationsPanel({ session, currentPage, onNavigatePage, onAddRectangle, onSelectText, selectionMode, blocked = false }: {
  ...
  return <div className="pdf-workbench-panel">
    <div className="pdf-workbench-actions">
      <Button disabled={blocked || !session.isWritable()} onClick={() => addComment("page_comment")}>{t("pdfReader.addPageComment", { page: currentPage + 1 })}</Button>
      <Button disabled={blocked || !session.isWritable()} onClick={() => addComment("document_comment")}>{t("pdfReader.addDocumentComment")}</Button>
      <span className="pdf-workbench-mode-group">
        <Button className="pdf-workbench-mode" aria-pressed={selectionMode === "area"} disabled={blocked || !session.isWritable()} onClick={onAddRectangle}>{t("pdfReader.addRectangle")}</Button>
        <Button className="pdf-workbench-mode" aria-pressed={selectionMode === "text"} disabled={blocked || !session.isWritable()} onClick={onSelectText}>{t("pdfReader.selectText")}</Button>
      </span>
    </div>
    <div className="pdf-workbench-history">
      <Button disabled={!state.canUndo || blocked} onClick={session.undo}>{t("pdfReader.undo")}</Button>
      <Button disabled={!state.canRedo || blocked} onClick={session.redo}>{t("pdfReader.redo")}</Button>
    </div>
    {blocked && <p role="status">{t("pdfReader.changedBlocked")}</p>}
    {blocked && Object.entries(state.drafts).filter(([id]) => !id.startsWith("style:")).map(([id, body]) => <div className="pdf-workbench-item" key={id}>
      <p>{t("pdfReader.unsavedDraft")}</p>
      <textarea aria-label={t("pdfReader.annotationBody")} value={body} readOnly />
      <Button onClick={() => copyDraft(id, body)}>{t("pdfReader.copyDraft")}</Button>
      <Button onClick={discard}>{t("pdfReader.discardPending")}</Button>
    </div>)}
    ...
    {failure && (state.dismissedNoticeId === failure.noticeId
      ? <div className="pdf-annotation-failure-status" role="status">
          ...
          <Button disabled={state.saving} onClick={() => void session.retry()}>{t("pdfReader.retrySave")}</Button>
          <Button disabled={state.saving} onClick={discard}>{t("pdfReader.discardPending")}</Button>
        </div>
      : <InlineMessage tone={failure.kind === "busy" ? "warning" : "danger"} title={t(failure.kind === "busy" ? "pdfReader.saveBusy" : "pdfReader.saveFailed")}>
          <IconButton variant="ghost" label={t("pdfReader.dismissSaveError")} icon={<Icon name="x" cls="ico-sm" />} onClick={session.dismissFailure} />
    ...
    {items.map((annotation) => {
      ...
      return <article className="pdf-workbench-item" id={`pdf-annotation-${annotation.id}`} key={annotation.id}>
        <button type="button" className="pdf-workbench-target" disabled={blocked || annotation.kind === "document_comment"} onClick={() => {
        ...
        <Button onClick={() => void copyAnnotationLink(annotation.id)}>{t("pdfReader.copyAnnotationLink")}</Button>
        ...
        return <>
          ...
          <label>{t("pdfReader.rectangleColor")} <input aria-label={t("pdfReader.rectangleColor")} disabled={blocked} type="color" value={style.color} onChange={(event) => { const next = { ...(styleDraftsRef.current[annotation.id] ?? style), color: event.target.value }; setStyle(next); commitStyleValue(next); }} onKeyUp={(event) => { if (event.key === "Enter") commitStyle(); }} /></label>
          <label>{t("pdfReader.rectangleOpacity")} <input aria-label={t("pdfReader.rectangleOpacity")} disabled={blocked} type="range" min="0.1" max="1" step="0.05" value={style.opacity} onChange={(event) => setStyle({ ...style, opacity: Number(event.target.value) })} onPointerUp={commitStyle} onKeyUp={commitStyle} /></label>
          <label>{t("pdfReader.rectangleLineWidth")} <input aria-label={t("pdfReader.rectangleLineWidth")} disabled={blocked} type="number" min="1" max="32" value={style.line_width} onChange={(event) => setStyle({ ...style, line_width: Number(event.target.value) })} onBlur={commitStyle} onKeyUp={(event) => { if (event.key === "Enter") commitStyle(); }} /></label>
        </>;
      })()}
        <textarea aria-label={t("pdfReader.annotationBody")} disabled={blocked} value={draft} onChange={(event) => session.setDraft(annotation.id, event.target.value)} />
        <div className="pdf-workbench-body" dangerouslySetInnerHTML={{ __html: mdWithMath(annotation.body) }} />
        <div className="pdf-workbench-item-actions">
          <Button disabled={blocked || !hasDraft} onClick={() => session.saveBody(annotation.id)}>{t("pdfReader.saveBody")}</Button>
          {hasDraft && <Button onClick={() => copyDraft(annotation.id, annotation.body)}>{t("pdfReader.copyDraft")}</Button>}
          <Button disabled={blocked || !session.isWritable()} onClick={() => session.remove(annotation.id)}>{t("pdfReader.deleteAnnotation")}</Button>
        </div>
      </article>;
    })}
