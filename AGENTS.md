...
**历程**（详见 `.kimi-code/memory/2026-09-01-development-log.md`）：Stage 1 TS 重构（2026-08-27）→ Stage 2 agent 接入（2026-08-27）→ Stage 3 存储统一 `./literatures` + 渲染 IR 三端共用 + webui 补齐（2026-09-01）→ Stage 3.1 摄入收窄 LaTeX + zip 上传 + 公式/表格修复（2026-09-02）→ Stage 4 计划页面（2026-09-02 关闭）→ Stage 5 LaTeX 解析线路重构（IR 化）（2026-09-09 关闭）→ Stage 6 阅读器四项修复（2026-09-09 关闭）。

**下一步**：**Stage 7 = 其余 known-issues**（tikz/re-upload 推广/CLI 未识别源建条目/task.due 语义/TOC 预览 `[cite:…]`/hyperlink-only xref 卡 + Stage 6 作者块·图尺寸的已知边界——见 `.kimi-code/memory/2026-09-09-stage6-roadmap.md` 推后事项节 + `2026-09-01-known-issues.md`）；**Stage 8 预告**：标记功能（渲染页面直接标记到精确位置，agent 可见）。Stage 6 已关闭（宽度流式上限 100ch/多引用 per-ref chip/右栏定位停稳校正/当前文献作者块（AASTeX·revtex·aa.cls 三族机械还原 + 平铺降级）；存量 7 篇离线重摄入、agent 输出逐字节冻结守住；smoke R1 顺手修了 revtex 粘贴 .bbl 宏汤与 \ensuremath KaTeX shim；定稿/里程碑/审查记录见 `.kimi-code/memory/2026-09-09-stage6-roadmap.md`）。Stage 4.1（agent 操作计划页面：CLI/skills 读写 `status/plans.json`）**推后**（数据层已预留：稳定 id/pretty JSON/watcher/plan.changed/CRUD 纯函数）。开放问题/推后事项（**全条目 re-upload**、**webui 独立应用原则**、CLI 未识别源建条目、**task.due 可晚于 plan.due**、**tikz/pgfplots 不渲染**）见 `2026-09-01-stage3x-roadmap.md` 推后事项节 + `2026-09-01-known-issues.md`。
